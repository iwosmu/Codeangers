...
export function FlowScreen({ session }: { session: BriefSession }) {
  ...
  return <main id="main-content" className="screen screen-flowchart">
    ...
    {flow.result && <>
      <div className="flow-summary"><div><span className="flow-live-dot" /> {graph!.nodes.length} tasks <span className="flow-summary-divider">/</span> {graph!.edges.length} dependencies</div><div><Clock3 size={14} aria-hidden="true" /><strong>{flow.result.validation.makespan_hours}h</strong> simulated duration</div><div><Users size={14} aria-hidden="true" /><strong>{flow.result.validation.total_person_hours}h</strong> total person-hours</div><span className="flow-summary-team">For {flow.result.team_size} people</span></div>
      <div ref={workbench} className={`flow-workbench ${expanded ? 'expanded' : ''}`} tabIndex={-1} role={expanded ? 'dialog' : undefined} aria-modal={expanded || undefined} aria-label="Task flow workspace">
        ...
        {current && <aside className="flow-inspector" aria-label="Selected task details"><div className="flow-inspector-eyebrow"><span>Task {String(current.id).padStart(2, '0')}</span><span style={{ color: colour(current.group) }}>{current.group}</span></div><h3>{current.label}</h3><div className="flow-task-facts"><span><Clock3 size={15} aria-hidden="true" /> {current.estimated_time_hours} hours</span><span><Users size={15} aria-hidden="true" /> {current.people_needed} {current.people_needed === 1 ? 'person' : 'people'}</span></div><p className="flow-description">{current.title}</p><div className="flow-inspector-section"><h4>Needs first <span>{prerequisites.length}</span></h4>{nodeLinks(prerequisites, 'No prerequisites. Ready to start when people are available.')}</div><div className="flow-inspector-section"><h4>Unlocks next <span>{unlocks.length}</span></h4>{nodeLinks(unlocks, 'No dependent tasks. This is an endpoint in the plan.')}</div><div className="flow-inspector-section"><h4>Task owners <span>{github.owners[current.id]?.length ?? 0}/{current.people_needed}</span></h4><p className="flow-muted">Choose {current.people_needed} {current.people_needed === 1 ? 'person' : 'people'}. Confirm interest and availability together.</p><div className="flow-owner-options">{members.map(member => {
          ...
          return <label key={member.id}><input type="checkbox" checked={checked} disabled={github.busy || github.started || (!checked && chosen.length >= current.people_needed)} onChange={() => { github.setOwners({ ...github.owners, [current.id]: checked ? chosen.filter(id => id !== member.id) : [...chosen, member.id] }); github.invalidate() }} /><span>{memberName(member.id)}<small>{coverage && coverage.level !== 'none' ? `${coverage.level === 'strong' ? 'Strong' : 'Some'} CV evidence · ${coverage.evidence_ids.join(', ')}` : 'Area evidence not stated'}</small></span></label>
        })}</div></div></aside>}
        </div>
      </div>
