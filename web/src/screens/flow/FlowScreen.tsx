...
export function FlowScreen({ session }: { session: BriefSession }) {
  ...
  useEffect(() => {
    ...
    const key = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Tab' && workbench.current) {
        const elements = Array.from(workbench.current.querySelectorAll<HTMLElement>('button:not(:disabled), input, [tabindex="0"]')).filter(el => el.getClientRects().length)
  ...
  return <main id="main-content" className="screen screen-flowchart">
    <header className="flow-heading"><div><p className="eyebrow">04 / Plan the work</p><h2>A clear path to building.</h2><p>Explore the work, follow its dependencies, and see what can begin together.</p></div><Button disabled={!ready || busy} onClick={() => void generateFlow()}><RotateCw size={15} aria-hidden="true" />{flow.busy ? 'Planning…' : flow.result ? 'Regenerate flow' : 'Generate task flow'}</Button></header>
